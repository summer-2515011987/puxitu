  ...
  methods: {
    ...
    dictDataSubmit() {
      if (this.dataShow && this.tablename) {
          dataManageTable.changeTable(this.tablename, this.tableDictData).then(response => {
          this.$message({
            message: '添加成功！',
            type: 'success'
          });
      ...
      showColumns() {
              dataManageTable.searchColumns(this.tablename).then(response => {
                  const result = response.data;
                  for (let i=0;i<result.length;i++){
                      // $("#showColumns").append('<p>'+(i+1)+' '+result[i].attnum+' '+
                      //     result[i].attname+' '+result[i].type+' '+result[i].description+'</p>');
                      this.tableDictData.push({ columnName:result[i].dbcolumn,type: result[i].columntype,
                          length:result[i].columnlength,
                          defaultValue: result[i].isnull,comment:result[i].description,issearch:result[i].issearch});
                  }
