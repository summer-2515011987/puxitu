<template>
  ...
  <el-card class="box-card h100p" >
    <div slot="header" class="clearfix">
      <el-page-header @back="$router.back();" content="分配资源"></el-page-header>
    </div>
    ...
    <el-container style="padding:0">
      <el-main style="padding-left: 0; padding-right: 0 ;padding-top: 0">
        <div v-for="(cate,index) in allResourceCate" :class="index===0?'top-line':null" :key="'cate'+cate.id">
          <el-row v-loading="listLoading" class="table-layout" style="background: #F2F6FC;">
            <el-checkbox v-model="cate.checked"
                         :indeterminate="isIndeterminate(cate.id)"
                         @change="handleCheckAllChange(cate)" style="float: left;">
              {{ cate.name }}
            </el-checkbox>
          </el-row>
          <el-row v-loading="listLoading" class="table-layout">
            <el-col :span="6" v-for="resource in getResourceByCate(cate.id)" :key="resource.id" style="padding: 4px 0">
              <el-checkbox v-model="resource.checked" @change="handleCheckChange(resource)" style="float: left">
                {{ resource.name }}
              </el-checkbox>
            </el-col>
          </el-row>
